...
2a. **Hosted mode specifics.**
   ...
   - **Remote baseline** requires the `Agent_SaXML_Export` script installed in the hosted file (a one-time, per-file setup). Probe with a trigger call; if it 404s, **offer the standard paste-in pair**: print `${CLAUDE_PLUGIN_ROOT}/templates/saxml-table.xml` and `templates/agent-saxml-export-v5.xml` on screen (table → layout → script order; walkthrough in the scaffold's `workflow/02` runbook) — no placeholders, the `SAXML` drop table travels with the script, record `dropTable - SAXML` / `dropLayout - SAXML` / `textField - ResponseText`. Fall back to the v4 interview (`${CLAUDE_PLUGIN_ROOT}/docs/guides/remote-file-agent-workflow.md` §3, template `templates/agent-saxml-export.template.xml`) only when the standard table can't land as-is; then record that install's chosen names instead. Config lands in `fm/fm-dc.json`; smoke-test either way. Once installed, trigger the export over OData and land the XML in `schema/ddrs/YYYY-MM-DD/` (this replaces step 5's local export; parse it per the fm-saxml skill).
