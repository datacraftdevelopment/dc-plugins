...
2a. **Hosted mode specifics.**
   ...
   - **Remote baseline** requires the `Agent_SaXML_Export` script installed in the hosted file (a one-time, per-file setup). Probe with a trigger call; if it 404s, **offer the setup interview** from `${CLAUDE_PLUGIN_ROOT}/docs/guides/remote-file-agent-workflow.md` §3 — ask before acting, choose/create the drop table (every file is different; nothing is hardcoded), walk the human layout step if needed, generate the script from `${CLAUDE_PLUGIN_ROOT}/templates/agent-saxml-export.template.xml` with that file's names, print it on screen for the paste, record the chosen names in `fm/fm-dc.json`, then smoke-test. Once installed, trigger the export over OData and land the XML in `schema/ddrs/YYYY-MM-DD/` (this replaces step 5's local export; parse it per the fm-saxml skill).
