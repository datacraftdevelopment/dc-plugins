...
- **Limits** (see the matrix): the success banner lies; Replace can't
  carry script step bodies; custom functions can't be patched at all (Add
  silently no-ops, Replace is officially unsupported — clipboard XML only);
  container-field adds silently no-op (datatype-level);
